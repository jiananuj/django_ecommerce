...
def user_register(request):
	...
	if request.method != 'POST':
		codes.append('first_case_run'+str(random.randrange(101010,989898)))
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			elif form.cleaned_data['otp'] != str(codes[0]):
				#print(form.cleaned_data['otp'])
				#print(str(codes[0]))
				err_succ['message'] = 'OTP do not match.'
				codes.clear()
				codes.append(random.randrange(101020, 989799))
				send_mail(
					'CurrencyKart Email Verification Code',
					'The 6 digits confirmation code is: ' + str(codes[0]),
					'progjava08@gmail.com',
					[to_id],
					fail_silently=False,
				)

				err_succ['message'] = 'Email Verification Code sent to your Email Id. Please enter correct.'
				#err_succ['status'] = 0
